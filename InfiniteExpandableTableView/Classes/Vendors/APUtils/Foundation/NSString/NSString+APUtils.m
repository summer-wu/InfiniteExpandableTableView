...
+(float)heightForString:(NSString *)string font:(UIFont *)font constrainedToSize:(CGSize)constrainedSize lineBreakMode:(NSLineBreakMode)lineBreakMode {
    return [self sizeForString:string font:font constrainedToSize:constrainedSize lineBreakMode:lineBreakMode].height;
}

-(NSString*)rightNChar:(NSUInteger)nchar{
    NSString * s=[self substringFromIndex:(self.length-nchar)?:0]; //必须大于0
    return s;
}
